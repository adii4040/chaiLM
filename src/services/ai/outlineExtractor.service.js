...
export async function extractBatchSegments(batches, concurrency = 5, sourceType = "document") {
  ...
  const coordinateRule = isMedia
    ? "5. ACCURATE TIMESTAMP COORDINATES: For each segment, provide the accurate `rangeStart` and `rangeEnd` numerical coordinates in whole integer SECONDS (e.g. 0, 120, 3600) and human-readable `rangeLabel` formatted strictly as MM:SS or HH:MM:SS (e.g. '00:00-05:30' or '01:15:00-01:35:00'). NEVER use page numbers for video/audio. Use the timestamps in the text.\n"
    : "5. ACCURATE INTEGER PAGE COORDINATES: For each segment, provide the accurate `rangeStart` and `rangeEnd` numerical coordinates and human-readable `rangeLabel`. For documents/PDFs (where coordinates are pages), `rangeStart` and `rangeEnd` MUST BE POSITIVE WHOLE INTEGERS (e.g., 1, 2, 3). NEVER output fractional or decimal page numbers like 7.4 or 9.99. Use the `[Source Location: Page X]` headers in the text to identify exact page boundaries.\n";

  console.log(`[Studio Outline Extractor] Extracting segments using model '${modelName}' from ${batches.length} batch(es) for ${sourceType} with concurrency limit = ${concurrency}...`);
  ...
  const tasks = batches.map((batch, index) =>
    limit(async () => {
      ...
      try {
        ...
        const isPageBatch = !isMedia && (batch.rangeLabel || "").toLowerCase().includes("page");

        // Validate, sanitize and clamp segment coordinates strictly against batch boundaries
        segments = segments.map((seg) => {
          let rStart = typeof seg.rangeStart === "number" ? Math.round(seg.rangeStart) : batch.rangeStart;
          let rEnd = typeof seg.rangeEnd === "number" ? Math.round(seg.rangeEnd) : batch.rangeEnd;
          ...
          if (isMedia) {
            rLabel = `${formatTs(rStart)}-${formatTs(rEnd)}`;
          } else if (isPageBatch) {
